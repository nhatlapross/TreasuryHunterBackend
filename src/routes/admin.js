...
router.post('/treasures', [
    ...
    try {
        ...
        // 🆕 ADD: Create treasure on blockchain too
        let blockchainResult = null;
        try {
            const admin = await User.findById(req.user.userId);
            if (admin && admin.encryptedPrivateKey) {
                console.log('⛓️ Adding treasure to blockchain registry...');

                const suiService = new SuiService();
                blockchainResult = await suiService.addTreasure(admin.encryptedPrivateKey, {
                    ...
                    coordinates: `${treasure.location.coordinates[1]},${treasure.location.coordinates[0]}`, // lat,lng
                    requiredRank: treasure.requiredRank,
                    rewardPoints: treasure.rewardPoints
                });

                console.log(`✅ Treasure added to blockchain: ${blockchainResult.transactionDigest}`);
            }
        } catch (blockchainError) {
            console.warn(`⚠️ Failed to add treasure to blockchain: ${blockchainError.message}`);
            // Continue without blockchain - treasure still created in database
        }
        ...
        res.status(201).json({
            ...
            data: {
                ...
                blockchain: blockchainResult ? {
                    onChain: true,
                    transactionDigest: blockchainResult.transactionDigest,
                    gasUsed: blockchainResult.gasUsed
                } : {
                    onChain: false,
                    reason: 'Admin wallet not configured or insufficient balance'
                }
            }
        });
